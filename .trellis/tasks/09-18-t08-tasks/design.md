...
本任务交付：tasks表/domain/public ports；Task/Attempt/Result/Review状态机；原子claim与DAG测试。字段、枚举与状态来源于对应模块计划和 data-model；命令名字、URI、principal/Grant/capability 来源于 command-catalog。T03 将其落为机器 Schema，本任务复用生成类型，不另造同义接口。

## 具体处理顺序

1. 实现create/ready/publish/claim与单owner current Attempt约束。
2. 实现block/resume准备态、submit/review/self/automated、cancel/orphan/recover。
3. 实现parent导航与blocks DAG独立，completed follow-up，精确子ResultRef。
4. 实现scope request/批准后撤Grant阻塞与批量restore_open端口。
