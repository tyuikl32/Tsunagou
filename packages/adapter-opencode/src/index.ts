...
export const adapterKind = "opencode" as const;

export interface OpenCodeEvidenceInput extends HostProbeInput {
...
function checks(input: OpenCodeEvidenceInput): readonly ConformanceCheck[] {
  return BASELINE_CAPABILITIES.map((name) => ({ name, ...(input.baseline?.[name] ?? { status: "unknown" as const }) }));
}
...
export const opencodeAdapter: HostAdapter = new OpenCodeAdapter();

export const createOpenCodeInstallationPlan = (installationId: string, profileRef: string, action: InstallationPlan["action"]): InstallationPlan =>
