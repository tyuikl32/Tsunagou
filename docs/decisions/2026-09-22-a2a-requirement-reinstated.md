...
4. 当前没有可验证的宿主反向唤醒 API，Agent Card 明确返回 `pushNotifications=false`、`streaming=false`、`wake=unsupported`；delivery 持久化和 pull 可恢复，不能伪造 host wake。
...
首版 A2A 已不再只是规划项；后续文档只能把上述最小 profile 标为已支持，并把 streaming/push/host wake 标为未支持，直到有可复现实证。
