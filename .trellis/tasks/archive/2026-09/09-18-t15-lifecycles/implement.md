...
- [x] 检查依赖 T13, T14 的完成证据，读取本任务 PRD/design/上下文与对应 Trellis spec。
- [x] 检查工作区现有改动，确认Schema/端口已有产物和版本；不得覆盖用户改动。
- [x] 使用 task.py start 显式开始；现在任务保持planning，不自动运行产品实施。

## 实施步骤

- [x] 实现精确revision/digest用户控制决定，不从宿主对话伪造批准
- [x] 实现completion当前Attempt收敛，completed立即事务生效，checkpoint异步屏障
- [x] 实现handoff/succession冻结/收敛、义务替换、新契约与残余风险
- [x] 实现新lineage unassigned/新runtime无旧授权、显式恢复任务；unknown追加Resolution

## 检查

- [x] 用户沉默无限持久不拒绝/失败
- [x] checkpoint失败保留completed，归档受阻但repair可用
- [x] 旧owner/session/grant在继任和reset后不能复活
- [x] 用户ceiling/项目完成等固定边界不能被main风险接受绕过
