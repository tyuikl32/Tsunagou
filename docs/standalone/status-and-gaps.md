...
| F17 | daemon 消息后空闲宿主未自动回合 | generic bridge 仍 pull-first；阶段 A managed Codex app-server 已在 disposable managed E2E 中完成 `message/send -> thread_started -> turn_started -> context__project_read -> inbox__claim/fetch/presented`，后台 watcher 会回填终态；daemon restart、loopback callback 和旧 epoch 拒绝均已验证；阶段 B 已通过用户提供的官方 Unix endpoint 恢复 Desktop-originated thread，并由真实 A2A 观察到 `thread_resumed -> turn_started`；终态依附着 thread 的 MCP/插件/审批环境，未观察到时保持 running | 消息持久化且下次 Agent pull 可见；managed host wake 的 callback、presentation、turn 终态分别有 evidence；Desktop attach 必须由用户提供 documented endpoint，turn_started 不能替代 presentation/turn_completed | Adapter 增强 |
