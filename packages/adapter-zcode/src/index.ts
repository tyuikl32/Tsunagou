...
export interface ZCodeEvidenceInput extends HostProbeInput {
...
export class ZCodeAdapter implements HostAdapter {
  ...
  public async probeCapabilities(input: HostProbeInput): Promise<readonly ConformanceCheck[]> {
    const evidence = input as ZCodeEvidenceInput;
...
export const zcodeAdapter: HostAdapter = new ZCodeAdapter();

export const createZCodeInstallationPlan = (installationId: string, profileRef: string, action: InstallationPlan["action"]): InstallationPlan =>
