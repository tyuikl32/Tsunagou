...
核验日期：2026-09-18（Windows 11，Python 3.13.13，Node 24.19.0）。这是 T02 的研究证据，不等同于四个正式适配器已交付。首发发布门禁只要求 Codex、OpenCode、DeepSeek Harness；ZCode 适配器和本矩阵记录保留，但正式基线与宿主演示后置。原始会话 ID、token、转录和本机私密路径不写入仓库；证据只保存 keyed digest、状态和可复现原因。
