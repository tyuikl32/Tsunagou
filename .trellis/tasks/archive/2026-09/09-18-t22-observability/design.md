...
本任务交付：evaluation模块、structlog与可选OTel；ExperimentDefinition/Run/Result/Report；A/B/C/D数据集和指标采集框架。字段、枚举与状态来源于对应模块计划和 data-model；命令名字、URI、principal/Grant/capability 来源于 command-catalog。T03 将其落为机器 Schema，本任务复用生成类型，不另造同义接口。

## 具体处理顺序

1. 实现脱敏事件投影/只读审计，权限过滤不可被main绕过。
2. 实现intervention/rework/latency/token来源与availability口径。
3. 实现固定实验定义digest、随机种子、预算/版本记录和原始结果引用。
4. 实现报告Job及统计，不删除失败样本；OTLP默认关闭。
