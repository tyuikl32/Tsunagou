...
- [ ] 四宿主各自 11 项共同基线真实证据；当前 `release_check.py` 明确阻断。
- [ ] Windows 完整发布回归及 macOS/Linux 实际 smoke 记录。
- [ ] A/B/C/D 各至少 5 次、另一宿主复验和公开实验报告。

发布前必须运行：`uv run pytest`、`corepack pnpm -r run check`、`corepack pnpm exec vitest run`、`python tools/docs/validate_docs.py`、`uv run python tools/dev/release_check.py`。最后一项失败时只能交付 diagnostic build，不能宣称四宿主正式支持；发布和 push 仍需用户授权。
